...
select
   ...
   coalesce(lag(cast(timestamp as date)) over wd,
   first_value(cast(timestamp as date)) over wd) as start_date,
   stage,
   ...
   case when stage_id = 108 and reason is null then 1 else 
        case when  stage_id > 101 then 0 end end deal_won
from raw_funnel
window wd as (
